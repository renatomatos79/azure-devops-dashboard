...
function getCardDefinition(appItem, releaseItem, persistItem) {
    ...
    $.ajax({
        ...
        data.environments.forEach((value, index) => {
            ...
            if (env === DEVELOPMENT) {
                getLastSucceededDeploymentInfo(appItem, definitionName, cssId, ENVIRONMENT_DEV1, value.id);
            } else if (env === QUALITY) {
                getLastSucceededDeploymentInfo(appItem, definitionName, cssId, ENVIRONMENT_QA1, value.id);
            } else if (env === STAGING) {
                getLastSucceededDeploymentInfo(appItem, definitionName, cssId, ENVIRONMENT_STG1, value.id);
            } else if (env === PRODUCTION) {
                getLastSucceededDeploymentInfo(appItem, definitionName, cssId, ENVIRONMENT_PRD1, value.id);
            } else {
...
function sortReleaseListAndTakeElements(items, count) {
    ...
    function orderbyDesc( a, b ) {
      if ( a.releaseText < b.releaseText ){
        return 1;
      }
      if ( a.releaseText > b.releaseText ){
        return -1;
    ...
    const addIndexToObect = function(item, i) {
      return { index: i, content: item };
    }

    const unMap = function(item) {
        return item.content;
      }
    
    const sortedList = items.sort(orderbyDesc)
                            .map((m, i) => addIndexToObect(m, i))
                            .filter( (f,i) => i <= max);


    if (count == 0) {
        return sortedList.map((m, i) => unMap(m));
    }

    let succeeded = sortedList.filter(f => f.content.status === true);
    let failed  = sortedList.filter(f => f.content.status === false);
    let result = [];

    if (succeeded.length > 0) {
        result.push(succeeded[0].content);
    }

    if (failed.length > 0) {
        result.push(failed[0].content);
    }
...
function addReleaseItem(list, item) {
    let hasSome = list.some(s => s.releaseText === item.releaseText);
    if (!hasSome) {
        list.push(item);
    }
    return list;

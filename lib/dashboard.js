    ...
    httpClient.get(uri, { params: { data: {
        ...
        response.data.value.forEach(item => {
            ...
            if (workItemHasBeenRequested === false) {
                ...
                getWorkItemDefinition(url, respAppId, respDefinitionId, respDefinitionName, respEnvironment, respBuildId, workItem);
            }
        });
    });
}

function getWorkItemDefinition(url, appId, definitionId, definitionName, environment, buildId, workItem){
    const httpClient = new HttpClient();

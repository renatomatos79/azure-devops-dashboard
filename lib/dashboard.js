...
function getReleaseDefinitions(appItem) {
    ...
    let searchContent = appItem.prefix;
    ...
    $.ajax({
        ...
        data.value.forEach( (value, index) => {
            //debugger;
            if (value.path.toLowerCase() === searchPath.toLowerCase() && value.name.toLowerCase().startsWith(searchContent.toLowerCase())) {
                releaseList.push({
        ...
        appItem.releaseList = releaseList;

        releaseList.forEach((value, index) => {
            let cssId = formatCssId(value.definitionName);
            addNewReleaseTableRow(value.definitionId, value.definitionName, cssId, color);
            getReleaseDefinition(value.definitionId, value.definitionName, truncateBuildNumber);
        });
 ...
 */
function getReleaseDefinition(definitionId, definitionName, truncateBuildNumber) {
    ...
    $.ajax({
        ...
        getLastSucceededDeploymentInfo(definitionName, cssId, ENVIRONMENT_DEV1, developmentDefinitionEnvironmentId, truncateBuildNumber);
        getLastSucceededDeploymentInfo(definitionName, cssId, ENVIRONMENT_QA1, qualityDefinitionEnvironmentId, truncateBuildNumber);
        getLastSucceededDeploymentInfo(definitionName, cssId, ENVIRONMENT_STG1, stagingDefinitionEnvironmentId, truncateBuildNumber);
        getLastSucceededDeploymentInfo(definitionName, cssId, ENVIRONMENT_PRD1, productionDefinitionEnvironmentId, truncateBuildNumber);
 ...
 */
function getLastSucceededDeploymentInfo(definitionName, cssId, environment, definitionEnvironmentId, truncateBuildNumber) {
        ...
        $.ajax({
            ...
            data.value.forEach((value, index) => {
                ...
                if (hasSucceededInfo === false && (value.deploymentStatus === SUCCEEDED || value.deploymentStatus === PARTIALLY_SUCCEEDED)) {
                    ...
                    setReleaseProperties(definitionName, cssId, environment, buildId, releaseName, definitionId, releaseId, status);
                    ...
                    //console.log(releaseName + " - buildId: " + buildId);
                    const environment2 = environment.slice(0, -1) + "2";
                    setReleaseProperties(definitionName, cssId, environment2, buildId, releaseName, definitionId, releaseId, status);
                    hasFailedInfo = true;
 ...
 */
function addNewReleaseTableRow(definitionId, releaseTitle, releaseId, color) {
    let defaultContent = { 
        title: releaseTitle,
        buildId: 0,
        definitionId: 0,
...
function setContent(releaseNameId, environment, content) {
    let search = releaseNameId + "-row";
    let filter = app.cards.filter(f => f.rowId === search);
    if (filter.length === 1) {
        let item = filter[0];
        item.content[environment] = content;
        // console.log(releaseNameId, "-", environment, ": ", item);
    }
}
 ...
 */
function setReleaseProperties(definitionName, releaseNameId, environment, buildId, releaseText, definitionId, releaseId, status) {
    if (releaseText) {
        ...
        const content = {
            ...
            slicedControlName: slicedControlName,
            status: status,
            uri: uri,
        ...
        setContent(releaseNameId, environment, content);
    }
}
...
VSS.ready(() => {
    ...
    // getReleaseDefinitions(SEACH_BIZ_PATH, SEACH_BIZ_PREFIX);

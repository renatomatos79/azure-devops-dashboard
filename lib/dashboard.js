...
// SEACH CRITERIA
const SEACH_SF_PATH = '\\3-SF';
const SEACH_SF_PREFIX = 'SF ';

const SEACH_BIZ_PATH = '\\1-BIZ';
const SEACH_BIZ_PREFIX = 'vNext - DEV BIZ ';
...
function getReleaseDefinitions(appItem) {
    ...
    $.ajax({
        ...
        $.each(data.value, function (index, value) {
            if (value.path === searchPath && value.name.startsWith(searchContent)) {
                releaseList.push({
...
function setReleaseProperties(definitionName, releaseNameId, environment, buildId, releaseText, definitionId, releaseId, status) {
    if (releaseText) {
        ...
        $("#" + controlName).attr("href", uriReleasePipeline); //release-pipeline
        // sets the dashboard title link.
        $("#" + slicedControlName + "-release-link").attr("href", uriReleaseLink);

    ...
    httpClient.get(uri, { params: { data: {
        ...
        if (principalList.length > 0) {
            ...
            principalList.forEach((releaseItem, index) => {
                ...
                let card = addCardDefinition(appItem, releaseItem);
                ...
                getCardDefinition(card, releaseItem, callbackCardAdded);
            });
 ...
 */
function getCardDefinition(card, releaseItem, callbackCardAdded) {
    ...
    // let cssId = formatCssId(owner.definitionName);
    ...
    httpClient.get(uri, { params: { data: {
        ...
        response.data.environments.forEach((value, index) => {
            ...
            if (env === DEVELOPMENT) {
                getLastSucceededDeploymentInfo(card, definitionName, ENVIRONMENT_DEV1, value.name, value.id, callbackCardAdded);
            } else if (env === QUALITY) {
                getLastSucceededDeploymentInfo(card, definitionName, ENVIRONMENT_QA1, value.name, value.id, callbackCardAdded);
            } else if (env === STAGING) {
                getLastSucceededDeploymentInfo(card, definitionName, ENVIRONMENT_STG1, value.name, value.id, callbackCardAdded);
            } else if (env === PRODUCTION) {
                getLastSucceededDeploymentInfo(card, definitionName, ENVIRONMENT_PRD1, value.name, value.id, callbackCardAdded);
            } else {
 ...
 */
function getLastSucceededDeploymentInfo(card, definitionName, environment, environmentFullName, definitionEnvironmentId, callbackCardAdded) {
    if (definitionEnvironmentId && definitionEnvironmentId > 0) {
        const uri = getURI("_apis/release/deployments");
        const truncateBuildNumber = card.app.truncateBuildNumber;
        ...
        httpClient.get(uri, { params: { data: {
            ...
            response.data.value.forEach((value, index) => {
                ...
                if (allowRelease) {
                    ...
                    setReleaseProperties(card, definitionName, environment, environmentFullName, buildId, releaseName, definitionId, releaseId, status, reason, callbackCardAdded);
                } 
                // else {
 ...
 */
 function setReleaseProperties(card, definitionName, environment, environmentFullName, buildId, releaseText, definitionId, releaseId, status, reason, callbackCardAdded) {
    if (releaseText) {
        ...
        const content = {
            ...
            contentFilter: app.title.trim().toLowerCase() + "_" +
                           definitionName.trim().toLowerCase() + "_" +

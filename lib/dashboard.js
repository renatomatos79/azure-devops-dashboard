...
function getReleaseDefinitionsForAppItem(appItem) {
    ...
    httpClient.get(uri, { params: { data: {
        ...
        const callbackCardUpdated = function(definitionId, environment, content){
            ...
            if (card) {
                let fullList = card.content[environment];
...
function getLastSucceededDeploymentInfo(appId, definitionId, definitionName, environment, environmentFullName, definitionEnvironmentId, callbackCardAdded) {
    if (definitionEnvironmentId && definitionEnvironmentId > 0) {
        ...
        httpClient.get(uri, { params: { data: {
            ...
            const definitionEnvironmentId = response.config.params.data.extra.definitionEnvironmentId;
            ...
            response.data.value.forEach((value, index) => {
                ...
                if (allowRelease) {
                    ...
                    setReleaseProperties(responseAppId, responseDefinitionId, responseDefinitionName, responseEnvironment, responseEnvironmentFullName, buildId, releaseName, fullReleaseName, releaseDefinitionId, releaseId, deploymentStatus, reason, callbackCardAdded);
                }
            });
        });
    }
 ...
 * 
 */
 function setReleaseProperties(appId, responseDefinitionId, responseDefinitionName, environment, environmentFullName, buildId, releaseText, releaseTextComplete, releaseDefinitionId, releaseId, deploymentStatus, reason, callbackCardAdded) {
    if (releaseText) {

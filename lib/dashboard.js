...
function getReleaseDefinitions() {
    ...
    $.ajax({
        ...
        $.each(releaseList, function (index, value) {
            ...
            addNewReleaseTableRow(value.definitionName, cssId, 'badge-info');
...
function getLastSucceededDeploymentInfo(definitionId, definitionName, cssId, environment, definitionEnvironmentId) {
    if (definitionEnvironmentId && definitionEnvironmentId > 0) {
        ...
        $.ajax({
            ...
            $.each(data.value, function (index, value) {
                const releaseName = value.release.name;
                ...
                if (hasSucceededInfo === false && (value.deploymentStatus === SUCCEEDED || value.deploymentStatus === PARTIALLY_SUCCEEDED)) {
                    ...
                    setReleaseProperties(definitionId, definitionName, cssId, environment, buildId, releaseName, definitionId, releaseId, status);
                    ...
                    //console.log(releaseName + " - buildId: " + buildId);
                    const environment2 = environment.slice(0, -1) + "2";
                    setReleaseProperties(definitionId, definitionName, cssId, environment2, buildId, releaseName, definitionId, releaseId, status);
                    hasFailedInfo = true;
 ...
 */
function setReleaseProperties(defId, defName, releaseNameId, environment, buildId, releaseText, definitionId, releaseId, status) {

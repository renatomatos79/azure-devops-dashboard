...
function getReleaseDefinitions(appItem) {
    ...
    $.ajax({
        ...
        if (principalList.length > 0) {
            ...
            let complementList = releaseList.filter(f => f.isPrincipal === false);
...
function maxVersion(versionA, versionB) {
    let sortedVersions = [versionA, versionB].sort();
...
function setContent(releaseId, environment, content) {
    ...
    if (filter.length === 1) {
        ...
        if (list.length === 0) {
            ...
            let statusItem = list.filter(f => f.status === status);
            if (statusItem.length === 0) {
                list.push(content);
            } else {
                let compareWith = statusItem[0];
                let max = maxVersion(content.releaseText, compareWith.releaseText);
                // get the max version
                if (max === content.releaseText) {
                    if (max === content.releaseText) {
                        let index = list.indexOf(compareWith);
                        list.splice(index, 1);
                        list.push(content);
                    }
                }

...
function getVersion(release) {
  ...
  function maxVersion(v1, v2) {
    // let _vn1 = getVersion(v1);
    // let _vn2 = getVersion(v2);    
    // let _v1 = prepareToCompare(_vn1, GROUP_LENGTH);
    // let _v2 = prepareToCompare(_vn2, GROUP_LENGTH);
    // let _c1 = makeVersionComparable(_v1, GROUP_SYMBOL_MAX_SIZE, GROUP_LENGTH);
    // let _c2 = makeVersionComparable(_v2, GROUP_SYMBOL_MAX_SIZE, GROUP_LENGTH);
    // return (_c1 >= _c2) ? v1 : v2;
    return (v1 >= v2) ? v1 : v2;
  }
...
function sortReleaseListAndTakeElements(items, count) {
    ...
    function compareReleases(a, b) {
        if (a === b) {
            ...
            // if (a.includes("2.62.0.6") && b.includes("2.66.0.9")) {
            //      debugger;
            //  }
            let mv = maxVersion(a, b);
            //console.log("a: ", a, " b: ", b, " max: ", mv);
            if (mv === a) {
                return -1;
            } else {
                return +1;
    ...
    function orderbyDesc(a, b) {
      ...
      return compareReleases(a.releaseNumber, b.releaseNumber);
    }
    ...
    let result = [];
    let lastSucceeded = null;
    let lastPending = null;
    let lastFailed = null;
    let lastAfterSucceeded = null;

    if (succeeded.length > 0) {
        lastSucceeded = succeeded[0];
        result.push(lastSucceeded);
    }

    // only pending
    if (pending.length > 0 && failed.length === 0) {
        lastPending = pending[0];
        lastAfterSucceeded = lastPending;
    }

    // only failed
    if (pending.length === 0 && failed.length > 0) {
        lastFailed = failed[0];
        lastAfterSucceeded = lastFailed;
    }

    // pending and failed
    if (lastPending !== null && lastFailed !== null) {
        let compare = compareReleases(lastPending.releaseNumber, lastFailed.releaseNumber);
        // -1: pending is higher than failed
        //  0: pending is equal to failed
    ...
    // there is no lastSucceeded: add lastAfterSucceeded if exists
    if (lastSucceeded === null && lastAfterSucceeded !== null) {
        result.push(lastAfterSucceeded);
    } 

    // there is lastSucceeded: add the highest( lastSucceeded or lastAfterSucceeded)
    if (lastSucceeded !== null && lastAfterSucceeded !== null) {
        let compare = compareReleases(lastSucceeded.releaseNumber, lastAfterSucceeded.releaseNumber);
        if (compare === 1) {
            result.push(lastAfterSucceeded);
        } 
    } 

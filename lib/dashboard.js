    ...
    if (definitionEnvironmentId && definitionEnvironmentId > 0) {
        ...
        $.ajax({
            ...
            data.value.forEach((value, index) => {
                ...
                if (value.deploymentStatus === SUCCEEDED || value.deploymentStatus === PARTIALLY_SUCCEEDED) {
                ...
                if (value.deploymentStatus === FAILED || (value.deploymentStatus === NOT_DEPLOYED && value.operationStatus === PENDING)) {
                    const buildId = value.release.artifacts[0].definitionReference.version.id;

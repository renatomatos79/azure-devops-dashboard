...
function getReleaseDefinitionsForAppItem(appItem) {
    ...
    httpClient.get(uri, { params: { data: {
        ...
        response.data.value.forEach( (value, index) => {
			if (value.path.toLowerCase() === searchPath.toLowerCase()) {
                ...
                if (isReleaseMemberItem) {
                    azureDB.addReleaseDefinition(value.id, value.name, value.path, value.url, isPrincipal, appItemId);
        ...
        if (principalList.length > 0) {
            // cards will be created only for items when the property isPrincipal is true
            principalList.forEach((releaseItem, index) => {
                ...
                const callbackCardUpdated = function(definitionId, environment, content){
                    ...
                    // TODO: pegar as ultimas versoes disponiveis e preencher os cards
                ...
                getCardDefinition(releaseItem, callbackCardUpdated);
            });
        ...
        httpClient.get(uri, { params: { data: {
            ...
            response.data.value.forEach((value, index) => {
                ...
                if (allowRelease) {
                    ...
                    setReleaseProperties(responseAppId, responseDefinitionId, responseDefinitionName, responseEnvironment, responseEnvironmentFullName, buildId, releaseName, releaseDefinitionId, releaseId, deploymentStatus, reason, callbackCardAdded);
                }
            });
        });
    }
 ...
 * 
 */
 function setReleaseProperties(appId, responseDefinitionId, responseDefinitionName, environment, environmentFullName, buildId, releaseText, releaseDefinitionId, releaseId, deploymentStatus, reason, callbackCardAdded) {
    if (releaseText) {

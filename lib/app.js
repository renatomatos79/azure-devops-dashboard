var app = new Vue({
    el: '#app',
    data: {
      title: 'Releases MediaHub - Overview v1.0.42',
      filtered: false,
      settings: {
        show: false,
        showMore: false,
      ...
      cards: [],
      apps: [
        { id: "biz",    path: "\\1-BIZ",    searchContent: "BIZ",    prefix: "vNext - DEV BIZ",     sufix: "BIZ Deployment with DSC",    title: "BIZ", visible: false, releaseList: [], isReleaseListLoaded: false, checkItems: false, color: "warning", truncateBuildNumber: false, isRunning: false },
        { id: "ott",    path: "\\2-OTT",    searchContent: "OTT",    prefix: "vNext - DEV OTT",     sufix: "OTT Deployment with DSC",    title: "OTT", visible: false, releaseList: [], isReleaseListLoaded: false, checkItems: false, color: "success", truncateBuildNumber: false, isRunning: false },
        { id: "sf",     path: "\\3-SF",     searchContent: "SF",     prefix: "SF",                  sufix: "",                           title: "SF", visible: false, releaseList: [], isReleaseListLoaded: false, checkItems: false, color: "info", truncateBuildNumber: true, isRunning: false },
        { id: "player", path: "\\4-PLAYER", searchContent: "Player", prefix: "vNext - DEV Player",  sufix: "Player Deployment with DSC", title: "PLAYER", visible: false, releaseList: [], isReleaseListLoaded: false, checkItems: false, color: "dark", truncateBuildNumber: false, isRunning: false },
        { id: "bom",    path: "\\5-BOM",    searchContent: "BOM",    prefix: "vNext - DEV BOM",     sufix: "BOM Deployment with DSC",    title: "BOM", visible: false, releaseList: [], isReleaseListLoaded: false, checkItems: false, color: "secondary", truncateBuildNumber: false, isRunning: false },
        { id: "azr",    path: "\\6-AZR",    searchContent: "AZR",    prefix: "vNext - DEV AZR",     sufix: "AZR Deployment with DSC",    title: "AZR", visible: false, releaseList: [], isReleaseListLoaded: false, checkItems: false, color: "danger", truncateBuildNumber: false, isRunning: false },
        { id: "ovx",    path: "\\7-OVX",    searchContent: "OVX",    prefix: "vNext - DEV OVX",     sufix: "OVX Deployment with DSC",    title: "OVX", visible: false, releaseList: [], isReleaseListLoaded: false, checkItems: false, color: "primary", truncateBuildNumber: false, isRunning: false }
      ],
    },
    computed: {
      hasAppRunning: function(){
        return this.apps.some(s => s.isRunning === true);
      },
      
      filteredCards: function () {
        let resultList = this.cards.filter(f => f.release.isPrincipal === true);
    ...
    methods: {
      ...
      onShowMoreClick: function(event, env) {
        if (event.target.attributes.length > 0 && event.target.attributes.cardid !== undefined) {
          ...
          let cards = this.cards.filter(f=>f.id === cardid);
          if (cards.length > 0) {
            let card = cards[0];
            if (env === 'dev') {

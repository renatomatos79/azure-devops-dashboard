...
var AzureDashBoardDB = function(){
  ...
  self.init = function(){
    ...
    DBOpenRequest.onupgradeneeded = function(event) {
      ...
      deploymentStore.createIndex("app", "app", { unique: false });
  ...
  self.addReleaseDefinition = function(id, name, path, url, isPrincipal, appId) {
    ...
    transaction.onerror = function(error) {
      ...
      Log.add(`${self.storeReleaseDefinitions} - ${json}! Transaction error! ${error.target.error.message}`);
    };
  ...
  self.addDeployment = function(content) {
    ...
    transaction.onerror = function(error) {
      ...
      Log.add(`${self.storeDeployments} - ${json}! Transaction error! ${error.target.error.message}`);
    };
  ...
  self.getDeploymentsByDefinitionName = function(definitionName, environment, callback){
    let result = [];
    let objectStore = dbInstance.transaction(self.storeDeployments).objectStore(self.storeDeployments);
    let index = objectStore.index("definitionName").openCursor(window.IDBKeyRange.only(definitionName));
    index.onsuccess = function(event) {
        let cursor = event.target.result;
        // if there is still another cursor to go, keep runing this code
        if(cursor) {
          const content = cursor.value;
          if (content.envGroup === environment) {
            result.push(content);
          }
